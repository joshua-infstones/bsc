...
func (w *worker) commitTransaction(tx *types.Transaction, coinbase common.Address, trackProfit bool, receiptProcessors ...core.ReceiptProcessor) ([]*types.Log, error) {
...
LOOP:
	for {
		...
		logs, err := w.commitTransaction(tx, coinbase, false, bloomProcessors)
		switch {
		case errors.Is(err, core.ErrGasLimitReached):
			// Pop the current out-of-gas transaction without shifting in the next from the account
			//log.Trace("Gas limit exceeded for current block", "sender", from)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if len(pending) != 0 || len(w.eth.TxPool().AllMevBundles()) != 0 {
		start := time.Now()
